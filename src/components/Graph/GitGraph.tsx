...
export const GitGraph = () => {
    ...
    const [highlightColor, setHighlightColor] = useState<string>('var(--text)');
        ...
        <div className="w-full h-screen relative overflow-hidden flex items-center justify-center pb-24 md:pb-0" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
            ...
            {!isLoading && (
                <>
                    ...
                    {/* Top Right Toggle for Home Page */}
                    <motion.div
                    ...
                    >
                        ...
                        {/* Motto with subtle glow and code formatting */}
                        <motion.p
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ duration: 1, delay: skipAnimation ? 0 : 2 }}
                            className="font-mono text-xs sm:text-sm mt-4 break-words pointer-events-auto cursor-default tracking-wider"
                            style={{
                                fontFamily: 'var(--font-mono)',
                                color: 'var(--accent)',
                                textShadow: '0 0 6px var(--accent), 0 0 12px var(--accent)',
                                opacity: 0.9,
                                animation: 'pulse-glow 3s ease-in-out infinite'
                            }}
                            onMouseEnter={(e) => {
                                e.currentTarget.style.textShadow = '0 0 8px var(--accent), 0 0 16px var(--accent)';
                                e.currentTarget.style.opacity = '1';
                            }}
                            onMouseLeave={(e) => {
                                e.currentTarget.style.textShadow = '0 0 6px var(--accent), 0 0 12px var(--accent)';
                                e.currentTarget.style.opacity = '0.9';
                            }}
                        >
                            &gt; git commit -m "<span style={{ color: 'var(--text-secondary)', textShadow: '0 0 4px var(--text-secondary)' }}>Where Curiosity Turns into Contribution</span>"
                        </motion.p>
                    ...
                    {!isMobile && (
                        <>
                            <motion.div
                                className="absolute bottom-0 left-1/2 -translate-x-1/2 h-24 w-[1px]"
