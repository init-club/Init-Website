/**
 To people who handle this codebase in the future. This modal handles blog submissions from two very different user types.

 1) Two Submission Paths: If a user is logged in, their blog is linked to their account via 'author_id' (their UUID
    from the users table). If a guest submits, 'author_id' is null and they must supply their name manually.
    The 'loggedInUser' state controls which path is taken. Do NOT remove this distinction.

 2) Roll Number is Hidden for Logged-In Users: When logged in, the Name and Roll Number fields are hidden from the
    form entirely. Their details come from their existing profile. Roll numbers are never displayed publicly anywhere.
    For guest submissions, roll number is still collected but only shown to admins.

 3) Honeypot Anti-Spam: There is a hidden 'website_url' input in the form that is invisible to real users (it has
    display:none and tabIndex={-1}). If a bot fills it in, the submission is rejected silently. Do NOT remove this
    field. Also don't accidentally give it a visible label or real styling.

 4) Whitelist / Public Blog Toggle: The modal checks 'allow_public_blogs' from site_settings on every open. If false,
    only logged-in users (who are automatically whitelisted via 'isWhitelisted') can submit. Admins can toggle this
    from the admin panel without a code deploy.

 5) Preview Mode: The "Preview" button toggles 'showPreview' state which renders the markdown content as HTML via
    dangerouslySetInnerHTML. This is intentional for rich previewing. The content is only stored and rendered on the
    blog detail page, not executed, so XSS risk is low but be careful if you extend this.
...
const WriteBlogModal = ({ isOpen, onClose, onSuccess }: WriteBlogModalProps) => {
  ...
  useEffect(() => {
    ...
    const checkSettingsAndAuth = async () => {
      try {
        // Fetch public blogs toggle
        const { data: settings, error: settingsErr } = await supabase
          .from('site_settings')
          .select('allow_public_blogs')
          .eq('id', 1)
          .single();

        if (!settingsErr && settings) {
          setAllowPublicBlogs(settings.allow_public_blogs);
        }
        ...
        console.error('Failed to resolve blog permission:', err);
      }
    };

    checkSettingsAndAuth();
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalAuthorName = loggedInUser ? loggedInUser.name : formData.author_name;

    if (!finalAuthorName || !formData.title || !formData.content) {
    ...
    try {
      ...
      const { error } = await supabase.from('blogs').insert({
        ...
        roll_no: loggedInUser ? loggedInUser.roll_no : (formData.roll_no || null),
        ...
        >
          ...
          >
            <div className="relative bg-black/95 rounded-2xl shadow-2xl border border-purple-500/40">
              ...
              {!allowPublicBlogs && !isWhitelisted ? (
                <div className="p-12 text-center flex flex-col items-center gap-6">
                  ...
                  <h2 className="text-2xl font-bold text-white tracking-wide uppercase">Submissions Restricted</h2>
                  <p className="text-gray-400 text-sm max-w-md leading-relaxed">
                    Blog submissions are currently restricted to whitelisted club members. If you are a member, please sign in with your GitHub account first.
                  </p>
                  <button
                    onClick={onClose}
                    className="px-6 py-2.5 bg-gray-900 border border-gray-800 hover:bg-gray-800 text-white rounded-xl font-bold text-sm transition-colors"
                  >
                    Close Modal
                  </button>
                </div>
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    {!loggedInUser ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            type="tel"
                            placeholder="Optionally Give Your Phone Number"
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        type="url"
                        placeholder="Link of your cover image (https://example.com/image.jpg)"
                    ...
                    <div className="space-y-2">
                      ...
                      {showPreview ? (
                        ...
                        <textarea
                          required
                          placeholder="Write your blog content here... (Markdown is supported, reading the guidelines if you don't know how+)"
